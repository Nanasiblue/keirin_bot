...
INPUT_HTML = Path("data") / "raw_probe" / "odds.html"
OUTPUT_CSV = Path("data") / "entries_from_kdreams.csv"
DEFAULT_RACE_ID = "2320260627010001"
...
def main() -> None:
    ...
    tables = pd.read_html(StringIO(INPUT_HTML.read_text(encoding="utf-8")))
    ...
    output = pd.DataFrame(rows).sort_values("car_no")

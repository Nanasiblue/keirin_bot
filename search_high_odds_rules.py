...
PRED = "data/direct_ticket_predictions_full_valid.csv"
RACE_SCORE = "data/predictions_valid_2026_is_over_50.csv"

OUT_RULES = "data/high_odds_rule_search.csv"
OUT_MONTHLY = "data/high_odds_rule_search_monthly.csv"


def find_race_score_col(df):
    for col in ["score", "pred_score", "prediction", "proba", "race_score", "is_over_50_score"]:
        if col in df.columns:
            return col
    numeric_cols = [
        c for c in df.columns
        if c not in ["race_id", "race_date", "date", "target"]
        and pd.api.types.is_numeric_dtype(df[c])
    ]
    if not numeric_cols:
        raise ValueError(f"race score column not found: {df.columns.tolist()}")
...
def summarize(sel, rule):
    ...
    return {
        ...
        "min_race_score": sel["race_score"].min() if tickets else 0,
...
def main():
    ...
    df = pd.read_csv(PRED, dtype={"race_id": str, "combination": str})
    ...
    rs = pd.read_csv(RACE_SCORE, dtype={"race_id": str})
    score_col = find_race_score_col(rs)
    print(f"race score column: {score_col}")

    rs = rs[["race_id", score_col]].rename(columns={score_col: "race_score"})
    rs["race_score"] = pd.to_numeric(rs["race_score"], errors="coerce").fillna(0)

    df = df.merge(rs, on="race_id", how="left")
    df["race_score"] = df["race_score"].fillna(0)
    ...
    odds_ranges = [
        ...
        ("odds_150_1000", 150, 1000),
        ("odds_200_2000", 200, 2000),
        ("odds_300_3000", 300, 3000),
        ("odds_100plus", 100, 999999),
        ("odds_200plus", 200, 999999),
    ]

    race_thresholds = [0.30, 0.40, 0.45, 0.50, 0.55, 0.60]
    score_thresholds = [0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80]
    expected_thresholds = [0, 1000, 3000, 5000, 10000]
    top_ns = [1, 2, 3, 5, 10]
    ...
    print("searching high odds rules...")
    for odds_name, lo, hi in odds_ranges:
        ...
        for race_th in race_thresholds:
            race_base = odds_base[odds_base["race_score"] >= race_th].copy()
            if race_base.empty:
                continue

            for score_th in score_thresholds:
                score_base = race_base[race_base["direct_ticket_score"] >= score_th].copy()
                if score_base.empty:
                    continue
                    ...
                    for metric in metrics:
                        for n in top_ns:
                            ...
                            rule = (
                                ...
                                f"_expected>={exp_th}_top{n}_by_{metric}"
                            )
    ...
    # 荒れモードはhit_countが少なすぎるものを上に出しすぎない
    result["enough_hits"] = result["hit_count"] >= 20

    result = result.sort_values(
        ["enough_hits", "roi_without_max_hit", "month_min_roi", "roi", "hit_count"],
    ...
    print(result.head(100).to_string(
        index=False,
        formatters={
            "roi": lambda x: f"{x:.2%}",
